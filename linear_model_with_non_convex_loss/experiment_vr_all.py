...
# For debug purpose
use_grad_check = (
    False  # For debug purpose (incrase compute time a lot) use numerical gradients
)
...
fixed_alpha_diana = float(
    os.getenv("fixed_alpha_diana", 0.0)
)  # step size for DIANA's for update aggregated shift
...
vr_batch_size_percentage = float(
    os.getenv("vr_batch_size_percentage", 1.0 / 100.0)
)  # VR Marina batch size
...
data.train_true_targets = 2 * (
    (data.train_true_targets - min_target) / (max_target - min_target) - 0.5
)

max_target = np.max(data.test_true_targets)
min_target = np.min(data.test_true_targets)
data.test_true_targets = 2 * (
...
if mpi_size == 1:
    ...
    print(
        f"relative error for compute gradient in R^{D}: ",
        np.linalg.norm(gc - ga) / np.linalg.norm(gc),
    )
    print(f"absolute error for compute gradient in R^{D}: ", np.linalg.norm(gc - ga))
    print(
        f"dot product",
        np.dot(ga.reshape(1, D), gc.reshape(D, 1))
        / np.linalg.norm(ga)
        / np.linalg.norm(gc),
    )
    sys.exit(0)

SamplePerNodeTrain = math.floor(
    data.trainObservations() / Workers
)  # Samples for each node for train
...
for n in range(Workers):
    ...
    if n == worker_id or mpi_rank == 0:
        ...
        data_new.train_samples = data.train_samples[
            n * SamplePerNodeTrain : (n + 1) * SamplePerNodeTrain, :
        ]
        data_new.train_true_targets = data.train_true_targets[
            n * SamplePerNodeTrain : (n + 1) * SamplePerNodeTrain, :
        ]
        ...
        rootprint(
            f"INFO: Worker {n+1} will process the following number of samples: ",
            data_new.trainObservations(),
        )
...
rootprint(
    f"!Last {(data.trainObservations() - SamplePerNodeTrain * Workers)} records have been ignored"
)
...
fi_grad_calcs_by_node = np.zeros(
    (KTests, KRounds, KMax)
)  # Evaluate number gradients for fi
...
for t in range(KTests):
    ...
    if i_use_marina:
        ...
        descr["p"] = min(
            1.0 / (1 + w),
            (batch_size_for_worker) / (batch_size_for_worker + SamplePerNodeTrain),
        )
        descr["gamma"] = 1.0 / (
            Ltask
            + (
                (w * Ltask * Ltask + (1 + w) * Ltask * Ltask / batch_size_for_worker)
                ** 0.5
            )
            * ((1 - descr["p"]) / (descr["p"] * Workers)) ** 0.5
        )
        ...
        descr["gamma"] = 1.0 / (
            10 * Ltask * (1 + w / Workers) ** 0.5 * (m ** (2.0 / 3.0) + w + 1)
        )  # Th.4
        descr["p"] = 1 / m
    ...
    rootprint(
        f"! Compressor for nodes in experiment {t+1}/{KTests} is: ",
        compressorForNode.name(),
    )
    ...
    if specified_algorihms != 1:
        print(
            f"node {mpi_rank}/{mpi_size}: Before Launch tests please specify only single algorthm! You incorrectly speciy {specified_algorihms}"
        )
        sys.exit(-1)
    # ==================================================================================================================================
    ...
    rootprint(
        f"  Launch test with {KMax} iterations p={p}, gamma={gamma} across {Workers} workers / test {t+1}:{KTests}"
    )
    ...
    if i_use_diana:
        for r in range(KRounds):
            ...
            for k in range(0, KMax - 1):
                if k % (KMax // 10) == 0:
                    rootprint(
                        f"Evaluation for round {r+1}/{KRounds} is completed by ",
                        k / KMax * 100.0,
                        "%",
                    )
                ...
                if mpi_rank == 0:
                    ...
                    xk_nodes[t, r, k + 1, :] = xk_nodes[t, r, k, :].reshape(
                        1, D
                    ) - gamma * gk_avg.reshape(1, D)
                ...
                if mpi_rank != 0:
                    ...
                    if cmd == "bcast_xk":
                        xk_nodes[t, r, k, :] = comm.bcast(
                            xk_nodes[t, r, k, :], root=0
                        )  # Obtain current iterate from master
                        gk_next = fFullGrad(
                            data_for_node[worker_id],
                            lamb,
                            xk_nodes[t, r, k, :].reshape(D, 1),
                            None,
                        )  # Compute local gradient
                        gk_next = gk_next.reshape(D, 1)

                        delta = gk_next - hk  # Shifted gradient
                        mk_i = compressorForNode.compressVector(
                            delta
                        )  # Compress shifted local gradient
                        ...
                        comm.send(
                            hk.reshape((D, 1)), dest=0, tag=0
                        )  # Send message to the master (just for numeric purpose we send hk)
                        # ===========================================================================
                        fi_grad_calcs_by_node[t, r, k] = (
                            1 * data_for_node[worker_id].trainObservations()
                        )
                        transfered_bits_by_node[t, r, k] = (
                            compressorForNode.last_need_to_send_advance
                            * component_bits_size
                        )
                        # ===========================================================================
        ...
        for r in range(KRounds):
            ...
            for k in range(1, KMax):
                ...
                if mpi_rank == 0:
                    ...
                    if (
                        testp < p
                    ):  # (1.0/batch_size_for_worker):      # Q: Maybe replace to "P"?
                        uk = 1
                    else:
                        uk = 0
                    ...
                    if uk == 1:
                        comm.bcast(
                            "bcast_xk_uk_1", root=0
                        )  # Broadcast xk to all workers (and uk)
                        comm.bcast(xk_nodes[t, r, k, :], root=0)
                    if uk == 0:
                        comm.bcast(
                            "bcast_xk_uk_0", root=0
                        )  # Broadcast xk to all workers (and uk)
                        comm.bcast(xk_nodes[t, r, k, :], root=0)
                    ...
                    # Take proximal SGD step
                    xk_nodes[t, r, k, :] = xk_nodes[t, r, k - 1, :].reshape(
                        1, D
                    ) - gamma * gk.reshape(1, D)
                ...
                if mpi_rank != 0:
                    ...
                    if cmd == "bcast_xk_uk_0" or cmd == "bcast_xk_uk_1":
                        ...
                        # L-SVRG-US
                        indicies = np.random.permutation(
                            data_for_node[worker_id].trainObservations()
                        )[0:batch_size_for_worker]
                        dbgprint(
                            f"sample {len(indicies)} out of {data_for_node[worker_id].trainObservations()}"
                        )
                        ...
                        gk_w = fBatchGrad(
                            data_for_node[worker_id],
                            lamb,
                            yk_nodes[t, r, k, :].reshape(D, 1),
                            indicies,
                            None,
                        )
                        full_grad_w = fFullGrad(
                            data_for_node[worker_id],
                            lamb,
                            yk_nodes[t, r, k, :].reshape(D, 1),
                            None,
                        ...
                        comm.send(
                            mk_i.reshape((D, 1)), dest=0, tag=0
                        )  # Send message to the master
                        hk = hk + fixed_alpha_diana * mk_i  # Update the local shift

                        if k < KMax - 1:
                            if cmd == "bcast_xk_uk_1":
                                yk_nodes[t, r, k + 1, :] = xk_nodes[t, r, k, :]
                            if cmd == "bcast_xk_uk_0":
                                yk_nodes[t, r, k + 1, :] = yk_nodes[t, r, k, :]
                        # ===========================================================================
                        if k == 0:
                            # need to take full grad at beginning
                            fi_grad_calcs_by_node[t, r, k] = data_for_node[
                                worker_id
                            ].trainObservations()
        ...
        for r in range(KRounds):
            ...
            for k in range(0, KMax - 1):
                ...
                if mpi_rank == 0:
                    ...
                    dbgprint(
                        f"master--{k}--gn--", np.linalg.norm(gk.reshape(D, 1)) ** 2
                    )
                ...
                if mpi_rank != 0:
                    ...
                    if cmd == "bcast_g_c1":
                        gk = comm.bcast(gk, root=0)
                        dbgprint(
                            f"woker.cur iteration {k} gradient: ", np.linalg.norm(gk)
                        )
                        ...
                        gk_next_to_send = fFullGrad(
                            data_for_node[worker_id],
                            lamb,
                            xk_nodes[t, r, k + 1, :].reshape(D, 1),
                            None,
                        )
                        ...
                        dbgprint(
                            f"worker--{k}--gn--", np.linalg.norm(gk.reshape(D, 1)) ** 2
                        )
...
alg_finish_time = time.time()

rootprint(f"Completed in: ", str(alg_finish_time - t_alg_4_start), " seconds")

# ======================================= Visualize solution==================================================
rootprint(
    f"INFO: Script '{__file__}' main compute part completed in: ",
    ((time.time() - t0) / 60.0),
    " minutes",
)
...
if mpi_rank == 0:
    ...
    """
    shape = (Workers + 1(Central), tests, rounds, max_iters)
    """
    ...
    for i in range(1, mpi_size):
        ...
        pid = status.Get_source()
        fi_grad_calcs_by_nodes[
            pid, :, :, :
        ] = data_mpi  # / (data_for_node[pid-1].trainObservations())

    # Take batch size from worker-1
    fi_grad_calcs_by_nodes[
        0, :, :, :
    ] = fi_grad_calcs_by_node  # /data_for_node[0].trainObservations()

    main_fig_p1 = plt.figure()
    main_fig_p2 = plt.figure()
    ...
    for t in range(KTests):
        ...
        if specified_algorihms != 1:
            print(
                f"node {mpi_rank}/{mpi_size}: Please specify only single algorthm! You incorrectly speciy {specified_algorihms}"
            )
            sys.exit(-1)
        ...
        for z in range(KMax):
            ...
            transfered_bits_dx[z] = np.mean(
                (transfered_bits_by_nodes[:, t, 0, z] - transfered_bits_mx[z]) ** 2
            )

        for z in range(KMax):
            fi_grad_calcs_mx[z] = np.mean(fi_grad_calcs_by_nodes[:, t, 0, z])
            fi_grad_calcs_dx[z] = np.mean(
                (fi_grad_calcs_by_nodes[:, t, 0, z] - fi_grad_calcs_mx[z]) ** 2
            )

        transfered_bits_mean = (
            np.sum(transfered_bits_by_nodes[:, t, 0, :], axis=0) / Workers
        )
        fi_grad_calcs_sum = np.sum(fi_grad_calcs_by_nodes[:, t, 0, :], axis=0)

        for i in range(1, KMax):
            transfered_bits_mean[i] = (
                transfered_bits_mean[i] + transfered_bits_mean[i - 1]
            )
        ...
        for k in KSamples:
            ...
            for n in range(Workers):
                ...
                fn_train_with_regulizer_grad.append(
                    np.linalg.norm(fFullGrad(data_for_node[n], lamb, xcur, None)) ** 2
                )
        ...
        if i_use_vr_diana or i_use_vr_marina:
            ...
            my["m_batch_size_for_worker_in_percentage_to_local_data"] = (
                vr_batch_size_percentage * 100.0
            )
        else:
        ...
        if i_use_vr_marina:
            prefix4algo = "vr_marina"
        if i_use_marina:
            prefix4algo = "marina"
        if i_use_vr_diana:
            prefix4algo = "vr_diana"
        if i_use_diana:
            prefix4algo = "diana"
        if i_use_gd:
            prefix4algo = "gd"
        ...
        rootprint(
            ...
            (np.linalg.norm(xk_nodes[t, 0, KMax - 1, :]) ** 2) / 2.0,
        )
        rootprint(
            f"final iterate for {t+1}/{KTests} test. it's l2 norm: ",
            np.linalg.norm(xk_nodes[t, 0, KMax - 1, :]),
        )
        # ===============================================================================
        # Figures
        # ===============================================================================
        ...
        if i_use_vr_marina:
            algo_name = f"VR-MARINA [$\\gamma$={gamma:g}, p={p:g}, batch={vr_batch_size_percentage*100:g}% of local data]. Compress: {compressor_name}"
        if i_use_marina:
            algo_name = (
                f"MARINA [$\\gamma$={gamma:g}, p={p:g}]. Compress: {compressor_name}"
            )
        if i_use_diana:
            algo_name = f"DIANA  [$\\gamma$={gamma:g}, $\\alpha$={fixed_alpha_diana}]. Compress: {compressor_name}"
        if i_use_vr_diana:
            algo_name = f"VR-DIANA [$\\gamma$={gamma:g},$\\alpha$={fixed_alpha_diana}]. Compressor: {compressor_name}"
        if i_use_gd:
            algo_name = f"GD [$\\gamma$={gamma:g}]"

        if i_use_vr_marina:
            short_algo_name = f"VR-MARINA {compressor_name}"
        if i_use_marina:
            short_algo_name = f"MARINA {compressor_name}"
        if i_use_diana:
            short_algo_name = f"DIANA {compressor_name}"
        if i_use_vr_diana:
            short_algo_name = f"VR-DIANA {compressor_name}"
        if i_use_gd:
            short_algo_name = f"GD"

        markevery = [
            int(mark_mult * KMax / 10.0),
            int(mark_mult * KMax / 13.0),
            int(mark_mult * KMax / 7.0),
            int(mark_mult * KMax / 9.0),
            int(mark_mult * KMax / 11.0),
            int(mark_mult * KMax / 13.0),
        ][t % 6]
        marker = [
            "x",
        ...
        if not i_use_gd:
            plt.loglog(
                get_subset(transfered_bits_mean, KSamples),
                fn_train_with_regul_loss,
                color=color,
                linestyle=linestyle,
                label=short_algo_name,
        ...
        if t == KTests - 1:
            ...
            plt.title(f"{test_name}")
            rootprint(
                "NAME:", f'"{test_name}", n={Workers}, d={D}; ' + main_description
            )
            plt.xticks()
            plt.yticks()
        ...
        if t == KTests - 1:
            plt.xlabel("Iteration")
            plt.ylabel("Oracle request for evaluate $\\nabla f_i(x)$ at iteration")
            plt.grid(True)
            plt.legend(loc="best")
        ...
        if t == KTests - 1:
            plt.xlabel("Iteration")
            plt.ylabel("Sent bits from node to master at iteration")
            plt.grid(True)
            plt.legend(loc="best")
            #           fig.suptitle(f'{test_name}')
            plt.title("f")
    ...
    rootprint(
        f"INFO: Script '{__file__}' totally completed in: ",
        ((time.time() - t0) / 60.0),
        " minutes",
    )
    # plt.show()
